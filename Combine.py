...
TWEET_DATA = pd.read_csv(
    "D:/Kuliah/KRISPI/py/Analisis/data/datasetSource/tweet-dataset-ptm-test.csv")
...
normalizad_word = pd.read_csv(
    "D:/Kuliah/KRISPI/py/Analisis/cleaning_source/singkatan.csv")
...
TWEET_DATA.to_csv(
    "D:/Kuliah/KRISPI/py/Analisis/data/datasetSource/tweet-dataset-ptm-test1.csv")

# *********************Levenshtein Distance Algorithm************************
for dirname, _, filenames in os.walk('D:/Kuliah/KRISPI/py/Analisis'):
    df = pd.read_csv(
        "D:/Kuliah/KRISPI/py/Analisis/cleaning_source/kamus_levenshtein.csv")
    sentences_df = df[['word']]
    sentences_df.head(10)

    def get_plain_vocabluary():
        sentencess = [word_tokenize(sentence['word'])
    ...
    def spelling_correction(sentence):
        ...
        vocwords = list(itertools.chain.from_iterable(
            [get_plain_vocabluary()]))
        for i, word in enumerate(splittedsentence):
            if (word not in vocwords and not word.isdigit()):  # ignore digits
                ...
                splittedsentence[i] = vocwords[levdistances.index(
                    min(levdistances))]
            else:
...
df.to_csv(
    "D:/Kuliah/KRISPI/py/Analisis/data/datasetSource/tweet-dataset-ptm-test1.1.csv")
...
def preprocessingTweet(wordTweets): return filterfalse(lambda x:
...
def sentimentProcess(dataset): return (dict(clean_tweet=x, sentiment_result=sentimentCalc(sentimentFinder(x, preprocessingTweet)))
                                       for x in dataset)


def sentimentCSV(fileName: str) -> csv:
    tweetDataset = pd.read_csv(
        'D:/Kuliah/KRISPI/py/Analisis/data/datasetSource/tweet-dataset-ptm-{}.csv'.format(fileName))
    ...
    with open('D:/Kuliah/KRISPI/py/Analisis/data/datasetSource/sentimentAnalysis-result-ptm-{}.csv'.format(fileName), 'w') as file:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            executor.map(writer.writerow, sentimentProcess(
                tweetDataset['tweet']))
...
def sentimentPlotSingleFile(fileName: str) -> plt:
    datasetResult = pd.read_csv(
        'D:/Kuliah/KRISPI/py/Analisis/data/datasetSource/sentimentAnalysis-result-ptm-{}.csv'.format(fileName))
...
if __name__ == "__main__":
    ...
    sentimentCSV("test1.1")
    print(findWeightSentiment.cache_info())

    # grafik untuk distribusi sentiment
    sentimentPlotSingleFile("test1.1")

# save the final result to the csv format
tweetDataset = pd.read_csv(
    'D:/Kuliah/KRISPI/py/Analisis/data/datasetSource/tweet-dataset-ptm-test1.1.csv')
result = pd.read_csv(
    'D:/Kuliah/KRISPI/py/Analisis/data/datasetSource/sentimentAnalysis-result-ptm-test1.1.csv')
...
final_result.to_csv(
    "D:/Kuliah/KRISPI/py/Analisis/data/datasetSource/final_result/sentimentAnalysis-result-ptm-final-test.csv")
